use crate::{layer::Layers, random_bias, random_weight, DnArrayAs, Ndarray};
...
impl Linear {
    ...
    pub fn __new__<'py>(
        ...
        let random_weight: Ndarray<IxDynImpl> =
            random_weight(in_features.into(), out_features.into()).expect("faild to make random weight");
        let random_bias: Ndarray<[usize; 1]> = if is_bias {
            let r_bias: ArrayBase<OwnedRepr<f32>, Dim<[usize; 1]>> =
                random_bias(out_features.into()).expect("faild to make random bias");
            r_bias
        } else {
    ...
    fn __str__(slf: &Bound<Self>) -> String {
        ...
        format!(
            "{}(in_features = {}, out_features = {}, bias={}, params={}) ",
            class_name,
            slf.borrow().shape.0,
            slf.borrow().shape.1,
            slf.borrow().is_bias,
